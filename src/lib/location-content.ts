...
export function getLocationPageData(slug: string): LocationPageData | null {
  ...
  const baseData = {
    ...
    description: loc.region === 'Greater London'
      ? `Man with a van in ${loc.name} from £26/hr. One verified mover quotes — free to submit, no spam calls. Rated 5.0/5 from 6 reviews.`
      : `Man with a van in ${loc.name} from £19/hr. One verified mover quotes — free to submit, no spam calls. Rated 5.0/5 from 6 reviews.`,
