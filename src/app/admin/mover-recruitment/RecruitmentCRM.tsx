...
type CompanyInput = Omit<
  Company,
  "id" | "dateAdded" | "contactHistory" | "documentsReceived"
>;
...
function findDuplicateCompany(
  ...
  const phone = normalizeComparable(data.phone || "");
  const email = normalizeComparable(data.email || "");
  const website = normalizeComparable(data.website || "");
  const businessName = normalizeComparable(data.businessName || "");

  return list.find((c) => {
    if (excludeId && c.id === excludeId) return false;
    return (
      (phone && normalizeComparable(c.phone) === phone) ||
      (email && normalizeComparable(c.email) === email) ||
      (website && normalizeComparable(c.website) === website) ||
      (businessName && normalizeComparable(c.businessName) === businessName)
    );
  });
}
...
function extractPhone(value: string) {
  const matches = value.match(/(?:\+44\s?|0)\d(?:[\s().-]?\d){8,10}/g) || [];
  return matches[0]?.replace(/\s+/g, " ").trim() || "";
}

function inferAreaContext(rawHeading: string): AreaContext {
  const heading = rawHeading.replace(/^\d+\.\s*/, "").trim();
  const lower = heading.toLowerCase();

  if (lower.includes("birmingham")) {
    return {
      city: "Birmingham",
      county: "West Midlands",
      coverageArea: "Birmingham",
    };
  }
  if (lower.includes("dudley") || lower.includes("halesowen")) {
    return {
      city: "Dudley",
      county: "West Midlands",
      coverageArea: "Dudley, Halesowen",
    };
  }
  if (lower.includes("coventry")) {
    return {
      city: "Coventry",
      county: "West Midlands",
      coverageArea: "Coventry",
    };
  }
  if (lower.includes("walsall")) {
    return {
      city: "Walsall",
      county: "West Midlands",
      coverageArea: "Walsall",
    };
  }
  if (lower.includes("cannock") || lower.includes("tamworth")) {
    return {
      city: "Cannock",
      county: "Staffordshire",
      coverageArea: "Cannock, Tamworth",
    };
  }
  if (lower.includes("uttoxeter")) {
    return {
      city: "Uttoxeter",
      county: "Staffordshire",
      coverageArea: "Uttoxeter",
    };
  }

  return {
    city: heading.replace(/\s+Area$/i, ""),
    county: "",
    coverageArea: heading.replace(/\s+Area$/i, ""),
  };
}

function createEmptyCompany(area: AreaContext): CompanyInput {
  return {
    businessName: "",
    contactName: "",
    phone: "",
...
function appendNote(company: CompanyInput, note: string) {
  const clean = note.trim();
  if (!clean) return;
  company.notes = company.notes ? `${company.notes}\n${clean}` : clean;
}

function parseMoverRecruitmentText(rawText: string): CompanyInput[] {
  const defaultArea: AreaContext = { city: "", county: "", coverageArea: "" };
  let area = defaultArea;
  let current: CompanyInput | null = null;
  const companies: CompanyInput[] = [];

  const finishCurrent = () => {
    if (!current || !current.businessName.trim()) return;
    current.businessName = current.businessName.trim();
    current.contactName = current.contactName.trim();
    current.phone = current.phone.trim();
    current.email = current.email.trim();
    current.website = cleanWebsite(current.website) || current.website.trim();
    current.city = current.city.trim() || area.city;
    current.county = current.county.trim() || area.county;
    current.coverageArea =
      current.coverageArea.trim() || area.coverageArea || current.city;
  ...
  rawText
    ...
    .forEach((line) => {
      ...
      if (/^\d+\.\s+/.test(line)) {
        finishCurrent();
        current = null;
        area = inferAreaContext(line);
        return;
      }

      const labelled = line.match(/^([A-Za-z\/ ]+):\s*(.+)$/);
      ...
      if (!current || (!isKnownLabel && labelled === null)) {
        finishCurrent();
        current = createEmptyCompany(area);
        current.businessName = line;
        return;
      }
      ...
      if (["owner", "owner/director", "director", "founder"].includes(label)) {
        current.contactName = value.replace(/\s*\([^)]*\)\s*/g, " ").trim();
        appendNote(
          current,
          `${label.replace(/\b\w/g, (m) => m.toUpperCase())}: ${value}`,
        );
        return;
      }

      if (label === "companies house") {
        appendNote(current, `Companies House: ${value}`);
        return;
      }

      if (label === "website") {
        const website = cleanWebsite(value);
        if (website) current.website = website;
        appendNote(current, `Website/source: ${value}`);
        return;
      }
      ...
      if (label === "contact") {
        ...
        appendNote(current, `Contact: ${value}`);
        return;
      }

      if (label === "linkedin") {
        if (!current.contactName && value && !/linkedin/i.test(value))
          current.contactName = value;
        appendNote(current, `LinkedIn: ${value}`);
        return;
      }

      if (label === "location") {
        const location = value.replace(/\s+border\.?$/i, "").trim();
        if (location) {
          if (/^operates\s+across/i.test(location)) {
            current.coverageArea = location
              .replace(/^operates\s+across\s+/i, "")
              .replace(/\.$/, "");
          } else {
            current.city = location.split(/[\/,&]/)[0].trim() || current.city;
            current.coverageArea = value;
          }
        }
        appendNote(current, `Location: ${value}`);
        return;
      }

      if (label === "notes") {
        appendNote(current, value);
      }
    });
...
const SAMPLE_DATA: Company[] = [
  {
    ...
    status: "Interested",
    notes:
      "Spoke on phone. Interested in joining. Needs to send insurance docs.",
    contactHistory: [
      {
        id: generateId(),
        date: "2026-06-05",
        time: "14:30",
        method: "Phone Call",
        notes: "Introduced platform. He was interested.",
        outcome: "Interested - waiting for docs",
      },
      {
        id: generateId(),
        date: "2026-05-20",
    ...
    contactHistory: [
      {
        ...
        time: "10:00",
        method: "Email",
        notes: "Welcome email sent. Account activated.",
        outcome: "Approved and active",
      },
      {
        id: generateId(),
        date: "2026-05-15",
        time: "16:00",
        method: "Phone Call",
        notes: "Verified insurance documents.",
        outcome: "Docs approved",
      },
    ],
    documentsReceived: [
...
export default function RecruitmentCRM() {
  ...
  const filtered = useMemo(() => {
    let result = companies.filter((c) => {
      ...
      const matchCounty =
        !countyFilter ||
        c.county.toLowerCase().includes(countyFilter.toLowerCase());
  ...
  const stats = useMemo(() => {
    ...
    const pending = companies.filter(
      (c) => c.status === "Applied" || c.status === "Interested",
    ).length;
    const contacted = companies.filter(
      (c) => c.status !== "Not Contacted",
    ).length;
  ...
  const reminders = useMemo(() => {
    ...
    companies.forEach((c) => {
      ...
      else if (new Date(c.nextFollowUpDate) <= new Date(addDays(todayStr(), 7)))
        upcoming.push(c);
    });
  ...
  const checkDuplicate = useCallback(
    (data: Partial<Company>, excludeId?: string) =>
      findDuplicateCompany(companies, data, excludeId),
    [companies],
  );
  ...
  const addCompany = useCallback(
    (data: CompanyInput) => {
      ...
      if (dup) {
        alert(
          `Duplicate detected: "${dup.businessName}" already exists (${dup.status}).`,
        );
        return false;
      }
  ...
  // Bulk add pasted mover leads
  const addCompanies = useCallback((records: CompanyInput[]) => {
    let added = 0;
    ...
    setCompanies((prev) => {
      ...
      records.forEach((record) => {
        const duplicate = findDuplicateCompany(next, record);
        if (duplicate) {
          duplicates.push(
            `${record.businessName} → duplicate of ${duplicate.businessName}`,
          );
          return;
        }
  ...
  const quickStatus = useCallback(
    (companyId: string, newStatus: Status) => {
  ...
  const sendRecruitmentEmail = useCallback(
    async (company: Company) => {
      ...
      try {
        ...
        logContact(company.id, {
          method: "Email",
          notes:
            "Sent recruitment email via CRM with why-join and apply-to-join links.",
  ...
  const uniqueCounties = useMemo(
    () => Array.from(new Set(companies.map((c) => c.county))).sort(),
    [companies],
  );
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link
                ...
                aria-label="Man and Van Club home"
              >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={exportCSV}
                className="hidden md:flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-accent transition-colors"
              >
            ...
            {paginated.map((c) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${STATUS_STYLES[c.status]}`}
                  >
                    {c.status}
                  </span>
                </div>
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <a
                    href={`tel:${c.phone}`}
                    className="p-2 bg-gray-50 rounded-lg text-primary"
                  >
                    <Phone size={16} />
                  </a>
                  <a
                    ...
                    <th
                      ...
                      onClick={() => {
                        ...
                        if (sortField === col.key)
                          ...
                          setSortField(col.key as keyof Company);
                          setSortDir("asc");
                        }
                      }}
                    >
                ...
                {paginated.map((c) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-text-secondary">
                      {c.city}
                      <br />
                      <span className="text-xs">{c.county}</span>
                    </td>
                    <td className="px-4 py-3">
                      <a
                        href={`tel:${c.phone}`}
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border ${STATUS_STYLES[c.status]}`}
                      >
                        {c.status}
                      </span>
                    </td>
            ...
            >
              <div className="p-8 md:p-12 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => setShowAddForm(false)}
                    className="p-2 hover:bg-gray-100 rounded-xl text-primary/60"
                  >
                    <X size={24} />
                  </button>
                </div>
                <AddCompanyForm
                  ...
                  onBulkSubmit={(records) => {
                    const result = addCompanies(records);
                    if (result.added > 0) {
                      const skipped = result.duplicates.length
                        ? `\n\nSkipped duplicates:\n${result.duplicates.join("\n")}`
                        : "";
                      alert(
                        `Imported ${result.added} compan${result.added === 1 ? "y" : "ies"}.${skipped}`,
                      );
                      setShowAddForm(false);
                    } else {
                      alert(
                        result.duplicates.length
                          ? `No new companies imported.\n\nDuplicates:\n${result.duplicates.join("\n")}`
                          : "No companies found to import.",
                      );
                    }
                  }}
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-text-secondary mt-1">
                      {selectedCompany.contactName} • {selectedCompany.city},{" "}
                      {selectedCompany.county}
                    </p>
                  </div>
                  <button
                    onClick={() => setShowDetail(false)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        ...
                        <InfoField
                          ...
                          isLink={`tel:${selectedCompany.phone}`}
                        />
                        <InfoField
                          label="Email"
                          value={selectedCompany.email}
                          isLink={`mailto:${selectedCompany.email}`}
                        />
                        <InfoField
                          label="Website"
                      ...
                      <div className="space-y-4">
                        ...
                        <InfoField
                          label="County"
                          value={selectedCompany.county}
                        />
                        <InfoField
                          label="Coverage Area"
                          value={selectedCompany.coverageArea}
                        />
                        <InfoField
                          label="Date Added"
                          value={selectedCompany.dateAdded}
                        />
                        <InfoField
                          label="Last Contact"
                          value={selectedCompany.lastContactDate || "—"}
                        />
                        <InfoField
                          label="Next Follow-Up"
                          value={selectedCompany.nextFollowUpDate || "—"}
                        />
                      </div>
                        ...
                        <div className="bg-[#F9F9F7] rounded-2xl p-4 border border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-text-secondary">
                                Send why-join and apply links to{" "}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                sendRecruitmentEmail(selectedCompany)
                              }
                        ...
                        <div className="md:col-span-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {selectedCompany.documentsReceived.map((doc) => (
                              <span
                                key={doc}
                                className="px-3 py-2 bg-green-50 text-green-700 rounded-xl text-[10px] font-black uppercase tracking-wider border border-green-200"
                              >
                                <CheckCircle2
                                  size={12}
                      ...
                      {selectedCompany.contactHistory.length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {selectedCompany.contactHistory.map((log) => (
                            <div
                              key={log.id}
                              className="bg-gray-50 p-4 rounded-xl border border-border"
                            >
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (doc) {
                              updateCompany(selectedCompany.id, {
                                documentsReceived: [
                                  ...selectedCompany.documentsReceived,
                                  doc,
                                ],
                              });
                            }
...
function AddCompanyForm({
  ...
  onBulkSubmit: (records: CompanyInput[]) => void;
  ...
  const parsedCompanies = useMemo(
    () => parseMoverRecruitmentText(bulkText),
    [bulkText],
  );
  const parsedDuplicates = useMemo(
    () =>
      parsedCompanies
        .map((company) => ({ company, duplicate: checkDuplicate(company) }))
        .filter((item) => Boolean(item.duplicate)),
    [checkDuplicate, parsedCompanies],
  );
  ...
  const handleBulkImport = () => {
    if (parsedCompanies.length === 0) {
      alert(
        "Paste a recruitment list first. I could not detect any company names yet.",
      );
      return;
    }
    onBulkSubmit(parsedCompanies);
    setBulkText("");
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="rounded-2xl border border-accent/20 bg-accent/5 p-4 space-y-3">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-xs font-black uppercase tracking-widest text-primary">
              Paste bulk mover list
            </h3>
            <p className="text-xs text-text-secondary mt-1">
              Paste the whole area-by-area list once. The CRM will split it into
              companies and fill city, county, coverage, owner, phone, email,
              website and notes where it can.
            </p>
          </div>
          {parsedCompanies.length > 0 && (
            <span className="shrink-0 rounded-full bg-white px-3 py-1 text-[10px] font-black uppercase tracking-widest text-accent border border-accent/20">
              {parsedCompanies.length} found
            </span>
          )}
        </div>
        <textarea
          ...
          className="w-full min-h-[170px] p-3 bg-white border-2 border-transparent focus:border-accent rounded-xl font-medium text-sm outline-none"
          placeholder={`Example:\n1. Birmingham Area\nJ H Removals and Waste Disposal LTD\nOwner: Jordan Hope\nCompanies House: #13554019\nWebsite: jh-removal.co.uk\nContact: 07956 356045 / jhremovers@gmail.com`}
        />
        {parsedCompanies.length > 0 && (
          <div className="rounded-xl bg-white border border-border overflow-hidden">
            <div className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-primary/40 border-b border-border">
              Import preview
            </div>
            <div className="divide-y divide-border max-h-48 overflow-y-auto">
              {parsedCompanies.slice(0, 8).map((company, index) => {
                const duplicate = parsedDuplicates.find(
                  (item) => item.company.businessName === company.businessName,
                )?.duplicate;
                return (
                  <div
                    key={`${company.businessName}-${index}`}
                    ...
                    <div>
                      ...
                      <div className="text-text-secondary">
                        {[
                          ...
                          .filter(Boolean)
                          .join(" • ") || "Details will be stored in notes"}
                      </div>
                    </div>
                    {duplicate && (
                      <span className="shrink-0 rounded-full bg-red-50 text-red-700 border border-red-200 px-2 py-1 text-[9px] font-black uppercase tracking-wider">
                        Duplicate
                      </span>
                    )}
                  </div>
              ...
              {parsedCompanies.length > 8 && (
                <div className="p-3 text-xs font-bold text-primary/50">
                  + {parsedCompanies.length - 8} more companies
                </div>
              )}
            </div>
          </div>
        ...
        <button
          ...
          className="w-full py-4 rounded-xl bg-primary text-white font-black uppercase tracking-widest text-xs disabled:opacity-40 disabled:cursor-not-allowed hover:bg-accent transition-colors"
        >
      ...
      <div className="relative py-2 text-center">
        <span className="bg-white px-3 text-[10px] font-black uppercase tracking-widest text-primary/30">
          or add one manually
        </span>
      ...
      {dupWarning && (
        <div className="bg-red-50 border border-red-200 p-4 rounded-xl text-red-700 text-sm font-bold">
          ...
          Possible duplicate: "{dupWarning.businessName}" already exists with
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full p-3 bg-gray-50 border-2 border-transparent focus:border-accent rounded-xl font-bold text-sm outline-none"
            placeholder="e.g. Birmingham, Solihull"
          />
        </div>
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-widest text-primary/40">
          ...
          <select
            ...
            className="w-full p-3 bg-gray-50 border-2 border-transparent focus:border-accent rounded-xl font-bold text-sm outline-none cursor-pointer"
          >
            {STATUSES.map((s) => (
              <option key={s} value={s}>
                {s}
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className="w-full p-3 bg-gray-50 border-2 border-transparent focus:border-accent rounded-xl font-bold text-sm outline-none min-h-[100px]"
          placeholder="Initial notes about this company..."
        />
      </div>
      <button
        type="submit"
        className="btn-orange w-full py-5 rounded-xl font-black uppercase tracking-widest"
      >
        Add Company
      </button>
    </form>
...
function ContactLogForm({
  ...
  onSubmit: (log: Omit<ContactLog, "id" | "date" | "time">) => void;
}) {
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          className="w-full p-3 bg-gray-50 border-2 border-transparent focus:border-accent rounded-xl font-bold text-sm outline-none min-h-[120px]"
